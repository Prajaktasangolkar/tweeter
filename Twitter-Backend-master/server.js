...
app.post("/signup", async (req, res) => {
  ...
  if (takenUsername) {
    ...
    // Make sure to include the 'email' field when creating a new user
    user.password = await bcrypt.hash(req.body.password, 10);
...
app.post("/savedata/:userName", async (req, res) => {
  const data = req.body;

  try {
    
    const filter = { /* Specify your criteria here, e.g., { _id: data.id } */ };

   
    const updatedData = await YourDataModel.findOneAndUpdate(filter, {
      $set: {
        dob: data.dob,
